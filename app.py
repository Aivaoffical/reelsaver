# ReelSaver Backend — RapidAPI Version
...
def get_tiktok_info(url):
    ...
    if nwm_hd:
        formats.append({'quality': 'HD — No Watermark', 'ext': 'mp4', 'url': nwm_hd, 'filesize': video.get('hd_size') or video.get('size')})

    nwm_sd = video.get('play')
    if nwm_sd and nwm_sd != nwm_hd:
        formats.append({'quality': 'SD — No Watermark', 'ext': 'mp4', 'url': nwm_sd, 'filesize': video.get('size')})

    wm = video.get('wmplay')
    if wm:
        formats.append({'quality': 'With Watermark', 'ext': 'mp4', 'url': wm, 'filesize': video.get('wm_size')})

    music = video.get('music')
    if music:
        formats.append({'quality': 'Audio Only (MP3)', 'ext': 'mp3', 'url': music, 'filesize': None})
...
def get_instagram_info(url):
    ...
    if video_url:
        formats.append({'quality': 'HD Video', 'ext': 'mp4', 'url': video_url, 'filesize': None})

    for item in media.get('media', []):
        u = item.get('url') or item.get('video_url')
        if u and u != video_url:
            formats.append({'quality': f"Version {len(formats)+1}", 'ext': 'mp4', 'url': u, 'filesize': None})
...
def proxy():
    ...
    try:
        ...
        resp = Response(stream_with_context(generate()), status=200, content_type=r.headers.get('Content-Type', f'video/{ext}'))
